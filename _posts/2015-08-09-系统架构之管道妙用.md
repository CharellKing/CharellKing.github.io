...
## 模块解耦
将之前写的模块（除了GUI模块）全部删除，重新再来。避免之前出现的两种角色（HumanRole、NetRole)混淆，将两者分开来；并且初步画出自己的架构图；为了把大部分的时间花在逻辑上（避免去修饰控件、以及处理控件消息），决定实现CMD的五子棋。要是CMD能够实现，GUI也就实现起来更加清晰以及容易一些了。好多工具以cmd模式存在，这样在外面可以随便套入任何的GUI，比如geeknote命令，可以结合emacs使用，同时也可以是vim。以下分别是人机对弈、网络对弈的架构图：
![人机对弈-架构图](/assets/picture/2015-08-09-人机对弈架构图.png)
![网络对弈-架构图](/assets/picture/2015-08-09-网络对弈架构图.png)
